...
OUT_PATH = f"Z:\\PythonProcess\\AGsurveys\\TESTDATA\\out\\"  # Path of files
...
def read_CSV(name):  # reads profile from CSV file.
    source = name  # Concatenate path and filename.
    results = []
    data_Struct = []
...
def grid_division(data_z, size_m):
    # os_tile_location = get_input_file()
    os_grid = read_ostile()
    data = data_z
    ...
    for j in range(0, len(data)):
        for k in range(0, len(grid)):
            if data[j].xCoord == grid[k][0] and data[j].yCoord == grid[k][1]:  # Checks if data point is within current OS tile
                write_singleline_CSV(f"{out}{read_and_compare_OSTiles(data[j], os_grid)}", data[j])  # Write data point to file
                print(j)
...
def find_structures(data):
    ...
    for datum in data:
        if datum.fc == "SD":
            write_singleline_CSV(out + "STRUCTURES_FOR_YOUR_BEACH_2022rev1", datum)
        count += 1
